...
export const spec: Spec = {
  ...
  signals: [
    ...
    { name: 'anim_value', update: "invert('time', eased_anim_clock)" },
    {
      ...
      update:
        '{unit: "", fields: animation_frame_tuple_fields, values: [anim_value ? anim_value : min_extent]}'
    },
    {
  ...
  marks: [
    {
      ...
      encode: {
        update: {
          ...
          description: {
            signal:
              '"fertility: " + (format(datum["fertility"], "")) + "; life_expect: " + (format(datum["life_expect"], "")) + "; country: " + (isValid(datum["country"]) ? isArray(datum["country"]) ? join(datum["country"], \' \') : datum["country"] : ""+datum["country"]) + "; year: " + (isValid(datum["year"]) ? isArray(datum["year"]) ? join(datum["year"], \' \') : datum["year"] : ""+datum["year"])'
          },
